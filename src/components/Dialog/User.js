    ...
    <Fragment>
      <div onClick={openHandler}>
        <Tooltip title={props.username} placement="top">
          <Typography variant="body2" className={classes.username} color="secondary">@{props.username}</Typography>
        </Tooltip>
      </div>
            ...
            <Fragment>
              <Typography variant="body2" className={classes.center}>{errors}</Typography>
                ...
                <Fragment>
                  ...
                  {isFollow ? (
                    <Button variant="contained" onClick={unfollowHandler} disabled={followLoading ? true : false} color="secondary">Unfollow
                      {followLoading &&
                        <div style={{ marginLeft: '10px' }}>
                          <CircularProgress size={10} variant="indeterminate" color="primary" />
                        </div>}
                    </Button>
                  ) : (
                      <Button variant="contained" onClick={followHandler} color="primary" disabled={followLoading ? true : false}>Follow
                        {followLoading &&
                          <div style={{ marginLeft: '10px' }}>
                            <CircularProgress size={10} variant="indeterminate" color="secondary" />
                          </div>}
                      </Button>
                    )}
                  <br />
