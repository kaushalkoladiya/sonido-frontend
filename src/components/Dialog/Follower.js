...
const Follower = ({ classes, track }) => {
  ...
  return (
    <Fragment>
      <Button variant="text" color="primary" onClick={openHandler}>Follower</Button>
      <Dialog onClose={closeHandler} fullWidth open={open} maxWidth="sm">
        ...
        <DialogTitle className={classes.center}> <Typography variant="h4">Followers</Typography></DialogTitle>
            ...
            <Fragment>
              <Typography variant="body2" className={classes.center}>{errors}</Typography>
                ...
                <Grid container spacing={2}>
                  {followers.map((fwr, key) => <User fromFollow user={fwr.from} key={key} />)}
                </Grid>
              )}
        </DialogContent>
      </Dialog>
